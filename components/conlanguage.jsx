...
function LanguageSelector({ conlang, setConlang }) {
  ...
  const handleOptionClick = (selectedLanguage) => {
    setConlang(selectedLanguage);
    setIsOpen(false);
  };

  const currentVersion = Language_Versions[conlang];
  ...
  return (
    <div className="relative inline-block text-left" ref={dropdownRef}>
      ...
      >
        {conlang} Version: {currentVersion}
      </button>
            ...
            {Object.keys(Language_Versions).map((languageName) => (
              <button
                ...
                className={`block w-full text-left px-4 py-2 text-sm
                            ${conlang === languageName ? "bg-black text-white" : "text-gray-200 hover:bg-[#FAFAFA] hover:text-black"}`}
                role="menuitem"
              >
